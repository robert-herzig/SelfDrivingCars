...
class CarRacing(gym.Env, EzPickle):
    ...
    def step(self, action):
        ...
        return self.state, step_reward, done, true_speed,{}
...
if __name__=="__main__":
    ...
    while True:
        ...
        gray_state, state_image, state_image_full = lane_detection.crop_gray(s, cropsize=68)
        gradient = lane_detection.edge_detection(gray_state)
        lane_left, lane_right = lane_detection.lane_detection(gradient, lane_left_old, lane_right_old)
        waypoints, lane1_points, lane2_points = lane_detection.waypoint_prediction(lane_left, 
                                                        lane_right,
                                                        num_waypoints=6,
                                                        parameter_bound_waypoints=1.0, 
                                                        way_type = "smoothed")
        #speed = lane_detection.extract_sensor_values(s, 1)
        print(speed)
        a[0] = lane_detection.lateral_control(0, 0, speed, 0, waypoints, 1, 0.5)
        a[1], a[2], former_error, sum_error = lane_detection.longitut_control(speed, waypoints, former_error, sum_error)
        lane_left_old = lane_left
        lane_right_old = lane_right
        print(a)
        if steps % 4000 == 0 or done:
            #print("\naction " + str(["{:+0.2f}".format(x) for x in a]))
            #print("step {} total_reward {:+0.2f}".format(steps, total_reward))
            lane_detection.plot_state_lane_wayp(lane1_points, lane2_points, waypoints, state_image_full, steps)
            #import matplotlib.pyplot as plt
            #plt.imshow(s)
            #plt.savefig(str(steps)+"test.jpeg")
        steps += 1
