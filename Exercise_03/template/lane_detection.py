...
class LaneDetection:
    ...
    def edge_detection(self, gray_image):
        ...
        '''
        gradient_sum = np.gradient(gray_image)
        gradient_sum = np.array(gradient_sum)
        gradient_sum = gradient_sum.sum(0)
        print("GRADIENT SUM:" )
        print(gradient_sum.shape)
        threshold = 0.1
        gradient_sum[gradient_sum <= threshold] = 0 # remove values smaller than threshold
        print(gradient_sum)
        #TODO: Set small values to zero
